const projects = [
  {
    id: 1,
    title: 'Portfolio',
    description: ['Portfolio', 'Front-end', '2023'],
    img: [
      { id: 1, url: 'https://firebasestorage.googleapis.com/v0/b/projects-a129a.appspot.com/o/Projects%2FPortfolio%2FprojectPortfolio%2FportfolioHeadline.png?alt=media&token=4099b8df-7499-4252-a601-465a33f17f3d', alt: 'Portfolio_Headline' },
      { id: 2, url: 'https://firebasestorage.googleapis.com/v0/b/projects-a129a.appspot.com/o/Projects%2FPortfolio%2FprojectPortfolio%2FportfolioWorks.png?alt=media&token=3a49feb4-493e-47f4-ac06-f7cc94505ca4', alt: 'Portfolio_Works' },
      { id: 3, url: 'https://firebasestorage.googleapis.com/v0/b/projects-a129a.appspot.com/o/Projects%2FPortfolio%2FprojectPortfolio%2FportfolioContact.png?alt=media&token=aaf93c4e-e07c-4159-baf0-d36db018e228', alt: 'Portfolio_Contact' },
    ],
    content: 'My portfolio is a window into my professional and creative journey. With an intuitive design, I showcase significant projects and key skills. Each element not only displays achievements but also tells the story behind them, revealing the passion that drives my work. The careful presentation of skills reinforces my commitment to excellence, demonstrating their value in both creative and professional contexts.',
    contentModal: 'My portfolio offers a glimpse into my journey, showcasing key projects with an intuitive design. It reveals the passion driving my work, demonstrating my commitment to excellence in both creative and professional contexts.',
    techno: ['html', 'javaScript', 'css', 'Bootstrap', 'react', 'redux'],
    live: 'https://mdg0410.github.io/Portfolio/',
    source: 'https://github.com/mdg0410/portfolio',
  },
  {
    id: 2,
    title: 'Js Capstone',
    description: ['Js_Capstone', 'Font-End', '2023'],
    img: [
      { id: 1, url: 'https://firebasestorage.googleapis.com/v0/b/projects-a129a.appspot.com/o/Projects%2FPortfolio%2FprojectJsCapstone%2FJsCapstoneHome.png?alt=media&token=40daacf7-ee14-4624-86d8-ffa06533cb67', alt: 'JsCapstone_Home' },
      { id: 2, url: 'https://firebasestorage.googleapis.com/v0/b/projects-a129a.appspot.com/o/Projects%2FPortfolio%2FprojectJsCapstone%2FJsCapstoneReservation.png?alt=media&token=82c27e61-6aa8-43a9-ba88-ee9265ae56d0', alt: 'JsCapstone_Comments' },
      { id: 3, url: 'https://firebasestorage.googleapis.com/v0/b/projects-a129a.appspot.com/o/Projects%2FPortfolio%2FprojectJsCapstone%2FJsCapstoneComments.png?alt=media&token=382af464-1725-47b9-b76d-a602954ccc3e', alt: 'JsCapstone_Reservation' },
    ],
    content: 'My Capstone project, crafted with peer-to-peer programming and Scrum methodology, stands out for its elegance and functionality. Implemented with Jest, Webpack, and a custom API, it simulates a cinematic web platform where users can discover movies, express preferences with likes, leave comments, and make reservations. This amalgamation of technologies and agile approaches creates a refined user experience, showcasing both my technical skills and my ability to manage projects effectively.',
    contentModal: 'My Capstone project, crafted with peer-to-peer programming and Scrum, simulates a cinematic web platform using Jest, Webpack, and a custom API. It showcases my technical skills and effective project management for a refined user experience.',
    techno: ['html', 'css', 'javaScript', 'jest', 'webpack', 'API'],
    live: 'https://hafiz1379.github.io/JS-capstone-project/dist/',
    source: 'https://github.com/hafiz1379/JS-capstone-project?tab=readme-ov-file',
  },
  {
    id: 3,
    title: 'Awesome Book',
    description: ['Awesome Book', 'Front-End', '2023'],
    img: [
      { id: 1, url: 'https://firebasestorage.googleapis.com/v0/b/projects-a129a.appspot.com/o/Projects%2FPortfolio%2FprojectAwesomeBook%2FawesomeBookList.png?alt=media&token=efd7df5c-0741-40ec-ae9d-cc3584e3833a', alt: 'awesomeBooks_List' },
      { id: 2, url: 'https://firebasestorage.googleapis.com/v0/b/projects-a129a.appspot.com/o/Projects%2FPortfolio%2FprojectAwesomeBook%2FawesomeBookContact.png?alt=media&token=0ccfb38f-6f3a-467d-9687-ab4881b2dd7c', alt: 'awesomeBook_Contact' },
      { id: 3, url: 'https://firebasestorage.googleapis.com/v0/b/projects-a129a.appspot.com/o/Projects%2FPortfolio%2FprojectAwesomeBook%2FawesomeBookAddBook.png?alt=media&token=79e5b621-e8a7-4902-af99-03ee40212f69', alt: 'awesomeBook_AddBook' },
    ],
    content: '"awesomeBooks" is a project focused on efficient book manipulation through classes, enabling the addition, deletion, and instant rendering in the DOM. This application provides an organized and modular structure for effective information management, offering a professional and seamless user experience when interacting with the book list.',
    contentModal: '"awesomeBooks" simplifies book handling with classes for easy addition, deletion, and instant DOM rendering. Its organized structure ensures smooth information management, delivering a professional user experience.',
    techno: ['html', 'css', 'javaScript', 'DOM'],
    live: 'https://mdg0410.github.io/Awesome-Books/',
    source: 'https://github.com/mdg0410/awesomeBookES6',
  },
  {
    id: 4,
    title: 'To Do List',
    description: ['To Do List', 'Front-End', '2023'],
    img: [
      { id: 1, url: 'https://firebasestorage.googleapis.com/v0/b/projects-a129a.appspot.com/o/Projects%2FPortfolio%2FprojectToDoList%2FtodolistAdd.png?alt=media&token=aab6073c-a936-474c-8b16-dba188f92963', alt: 'toDoList_Add' },
      { id: 2, url: 'https://firebasestorage.googleapis.com/v0/b/projects-a129a.appspot.com/o/Projects%2FPortfolio%2FprojectToDoList%2FtodolistCheck.png?alt=media&token=09b24fba-321c-4b38-8ede-bc2d91b8af07', alt: 'toDoList_check' },
    ],
    content: 'This new project is a minimalist task list where we implemented the use of Webpack. It stands out for the integration of localstorage and JSON, providing efficient data management and an enhanced, professionally crafted user experience.',
    contentModal: 'This minimalist task list project with Webpack integrates localstorage and JSON for efficient data management and a polished user experience.',
    techno: ['html', 'css', 'javaScript', 'DOM'],
    live: 'https://mdg0410.github.io/to-Do-List/',
    source: 'https://github.com/mdg0410/To-Do-List',
  },
];

export default projects;
